...
class Kaggle2(nn.Module):
    def __init__(self, num_classes):
        ...
        self.conv2 = nn.Sequential(
            nn.Conv2d(64, 128, (1, 7), 1, 1), nn.BatchNorm2d(128), nn.ReLU()
        )

        self.conv3 = nn.Sequential(
            nn.Conv2d(128, 256, (1, 10), 1, 1), nn.BatchNorm2d(256), nn.ReLU()
        )

        self.conv4 = nn.Sequential(
            nn.Conv2d(256, 512, (7, 1), 1, 1), nn.BatchNorm2d(512), nn.ReLU()
        )
...
class MLP_(nn.Module):
    def __init__(self, sincnet_options: dict, num_classes: int):
        ...
        self.fc_1 = nn.Sequential(nn.Dropout(0.2), nn.Linear(45600, 2048), nn.PReLU())

        self.fc_2 = nn.Sequential(nn.Dropout(0.2), nn.Linear(2048, 1024), nn.PReLU())

        self.fc_3 = nn.Sequential(nn.Dropout(0.2), nn.Linear(1024, 512), nn.PReLU())

        self.fc_last = nn.Sequential(nn.Dropout(0.2), nn.Linear(512, num_classes))

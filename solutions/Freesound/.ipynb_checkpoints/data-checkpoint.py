...
class Dataset_Train(Dataset):
    ...
    def __getitem__(self, idx):
        try:
            if self.preload:
                audio = self.train_csv[
                    idx
                ]  ##in this case its will be just a list on numpy arrays
            else:
...
class Dataset_Train_Mixmatch(Dataset):
    ...
    def __len__(self):
        return (
            ...
            + (len(self.base_dataset) // self.mix_samples) * self.mix_rounds
        )
    ...
    def __getitem__(self, idx):
        if idx < len(self.base_dataset):
            ...
            if (
                len(self.samples_to_process)
                < len(self.base_dataset) // self.mix_samples
            ):
                self.samples_to_process = set(range(len(self.base_dataset)))
...
def get_tables():
    ...
    train_curated["fname"] = train_curated["fname"].apply(
        lambda x: f"{dataset_dir}/{x}"
    )
